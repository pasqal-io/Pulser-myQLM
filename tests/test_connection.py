...
import pytest
from qat.core import Batch, Result, BatchResult
...
from qat.qpus import RemoteQPU, PyLinalg
from qat.comm.exceptions.ttypes import QPUException
import pulser
from pulser_myqlm import PulserQLMConnection, FresnelQPU, IsingAQPU
    ...
    def __init__(self, 
        ...
        **kwargs: dict[str, typing.Any]
        ...
        self.available_qpus = {
            ...
            "RemoteFresnelQPU":RemoteFresnelQPU if not self.wrong_remote_fresnel_qpu else WrongRemoteFresnelQPU,  # Has a device in its descr
        }
...
@mock.patch(
    "pulser_myqlm.connection.qat.qlmaas.QLMaaSConnection", MockQLMaaSConnection
)
def test_available_devices():
    ...
    with pytest.raises(
        QPUException,
        match=f"TTransportException: Could not connect to any of "
    ):
    ...
    assert mock_conn.fetch_available_devices() == {"RemoteFresnelQPU":FresnelQPU(None).device}
...
def test_seq_submission():
    ...
    with pytest.raises(
        ValueError, 
        match="The Sequence's device AnalogDevice doesn't match"
    ):
        mock_conn.submit(seq)
    # This is also raised if mimic_qpu is set to True
    with pytest.raises(
        ValueError,
        match="does not match any of the devices currently available"
    ):
        mock_conn.submit(
            seq,
            mimic_qpu=True
    ...
    assert type(res) == pulser.backend.remote.RemoteResults
    ...
    with pytest.raises(QPUException, match="must be defined from one of the calibrated layouts"):
        mock_conn.submit(seq, job_params=[JobParams(runs=100, variables={})])
    # Using a layout from calibrated layouts
    seq._set_register(seq, 
    ...
    assert type(res) == pulser.backend.remote.RemoteResults
    assert res.batch_id == "0"
    assert res.job_ids == ["0"]
    assert res.get_batch_status() == pulser.backend.remote.BatchStatus.DONE
    assert res.get_available_results() == {"0":Result()}
